...
class Cache:
    '''
    ### Description:
        The Cache class describes the overall cache, implemented as a hash table.
        It contains three CacheLists which actually store the ContentItems. 
        Hash values of 0 correspond with the first CacheList (L1), 1 with L2, and 2 with L3

        All methods in the Cache class will call a corresponding method in the CacheList
        class. For example, the insert method calls the put method from the CacheList class.
    ...
    def insert(self, content, evictionPolicy):
        ...
        self.hierarchy[h].put(content, evictionPolicy)
    ...
    def retrieveContent(self, content):
        ...
        x = self.hierarchy[h].find(content)
    ...
    def updateContent(self, content):
        ...
        # find the content in the appropriate linked list
        x = self.hierarchy[h].update(content)
